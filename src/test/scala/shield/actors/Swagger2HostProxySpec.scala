...
class Swagger2HostProxySpec extends TestKit(ActorSystem("testSystem", ConfigFactory.parseString(
  """
  ...
  "HostProxy" must {
    ...
    "auto fetch swagger docs on start" in {
      ...
      val ref = TestActorRef(new HostProxy(serviceLocation, Swagger2ServiceType, settings.DefaultServiceLocation, _ => Future.successful(HttpResponse(StatusCodes.OK))) {
    ...
    "shed requests for a consistently failing endpoint" in {
      ...
      val ref = TestActorRef(new HostProxy(serviceLocation, Swagger2ServiceType, settings.DefaultServiceLocation, _ => {
        upstreamRequests += 1
        upstreamResponse
      }) {
        override def buildFetcher() = testFetcher
    ...
    "scrubs requests before forwarding them upstream" in {
      ...
      val ref = TestActorRef(new HostProxy(serviceLocation, Swagger2ServiceType, settings.DefaultServiceLocation, r => {
        latestRequest = r
    ...
    "broadcast state upon swagger fetch attempt" in {
      ...
      val testFetcher = new NeverFetcher
      val ref = TestActorRef(Props(new HostProxy(serviceLocation, Swagger2ServiceType, settings.DefaultServiceLocation, _ => Future.successful(HttpResponse(StatusCodes.OK))) {
        override def buildFetcher() = testFetcher
    ...
    "broadcast state upon endpoint breaker open" in {
      ...
      val parent = TestProbe()
      val upstreamResponse = Future.successful(HttpResponse(StatusCodes.InternalServerError))
      val ref = TestActorRef(Props(new HostProxy(serviceLocation, Swagger2ServiceType, settings.DefaultServiceLocation, _ => upstreamResponse) {
        override def buildFetcher() = testFetcher
      }), parent.ref, "hostProxy")
